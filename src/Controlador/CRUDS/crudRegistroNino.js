...
function crud() {
  ...
  var tabla = "presentacion_ninos";
  this.ingresar = (datos, callback) => {
    var consulta = "INSERT INTO `presentacion_ninos` (`ci`, `nombre`, `apellido_paterno`, `apellido_materno`, `nombre_padre`, `nombre_madre`, `fecha_nac`, `fecha_presentacion`, `num_reg_civil`, `fecha_partida`, `nombre_pastor`) VALUES (";
    const coma = ",";
    var strings = [0, 1, 1,
      1, 1,1,
      1, 1, 1,
  ...
  {
    this.conexion.query("SELECT * FROM presentacion_ninos WHERE ci= "+id_ci+";", (err, respuesta, fields)=>
    {
      if(!err)
      {
