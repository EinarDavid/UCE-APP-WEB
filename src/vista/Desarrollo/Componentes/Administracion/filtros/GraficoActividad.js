...
import POAGENERAL from "../Poa/poaGeneral";
...
export const GraficoActividad = ({ iglesia }) => {
  ...
  const ActualizarGrafico = () => {
    console.log("select", selectorIglesia.current.value);
    ...
    <div>
      <Form.Group as={Col} controlId="formGridEmail">
        <Form.Control
          ...
          required
          ref={selectorIglesia}
          onChange={() => ActualizarGrafico()}
        >
          <option value="Default" hidden>
            Seleccione la Iglesia
          </option>
          {iglesia.map((iglesia) => {
            return (
              <option
      ...
      {!iglesiaSeleccionada ? (
        <div>
          <h1 className="TituloGraficos">
            Cantidad de Actividades de las Iglesias
          </h1>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart
              data={listaGraficoIglesia}
        ...
        <div>
                ...
                {graficoVisible.map((actividad, i) => {
                  return (
                    <div className="ListActividad" key={actividad.Id}>
                      <img
                        src={
                          "/fotos/Iglesias/Actividad/" + actividad.FotoActividad
                        }
                        className="ImageActividad"
                      ></img>
                      <div className="DescriptionActividad">
                        <p className="TitleActividad">
                          {actividad.Titulo + ": " + actividad.Inicio}{" "}
                        </p>
                        <div className="CantAsistencia">
                          <div className="ContAsistencia">
                            <div className="Icon-Text">
                              <img
                                src="./Icons/Icons.png"
                                className="ImageCardActividad"
                              ></img>
                              <p className="NumAsistencia">
                                {actividad.ContAsistire}
                              </p>
                            </div>
          ...
          <POAGENERAL id={selectorIglesia.current.value} iglesia={iglesia} />
        </div>
      )}
    </div>
  );
